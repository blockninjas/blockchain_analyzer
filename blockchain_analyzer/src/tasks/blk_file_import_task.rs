  ...
  fn run(
    ...
    blk_files.par_iter().for_each(|blk_file| {
      ...
      let transaction_result = db_connection
        .transaction(|| import_blk_file(&db_connection, blk_file, blocks));
...
fn import_input(
  ...
  if is_segwit_tx {
    let script_witness_item_repository =
      ScriptWitnessItemRepository::new(db_connection);
...
mod test {
  ...
  pub fn genesis_block_is_imported_correctly() {
    ...
    db_connection.test_transaction::<_, Error, _>(|| {
      // When
      let _ =
        import_blk_file(&db_connection, "blk00000.dat", blocks.into_iter())
          .unwrap();
